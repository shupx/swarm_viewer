...
migrateApp(path.join(ROOT_DIR, 'main-app'), false);

// 2. Migrate Sub App
console.log('Migrating sub-app-demo...');
migrateApp(path.join(ROOT_DIR, 'sub-app-demo'), true);

// 3. Write Sub App main.tsx and public-path.js
console.log('Updating sub-app-demo entry points...');
fs.writeFileSync(path.join(ROOT_DIR, 'sub-app-demo/src/public-path.js'), `
if (window.__POWERED_BY_QIANKUN__) {
  __webpack_public_path__ = window.__INJECTED_PUBLIC_PATH_BY_QIANKUN__;
}
`);

fs.writeFileSync(path.join(ROOT_DIR, 'sub-app-demo/src/main.tsx'), `
import './public-path';
